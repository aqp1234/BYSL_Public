...
const setCalendarData = async (year, month) => {
    ...
    .then((data) => {
        console.log(data);
        data.calendars_map.map(calendar_sets => {
            let start_date = new Date(calendar_sets.calendar.start_date);
            let end_date = new Date(calendar_sets.calendar.end_date);
            for(let i = 0; i < Math.abs(end_date - start_date) / (1000 * 3600 * 24) + 1; i++){
                ...
                if(calendar_id_span.parentNode.childElementCount < 4){
                    let calHtml = "";
                    calHtml += `<div>
                    <a class='calendar' href='#calendar_layer' data-calendar_id=${calendar_sets.calendar._id} onclick=setCalendarLayerData(this) style='font-size: 13px; font-weight: bolder; color: #515151; text-decoration: none; margin-top: 10px;'>
                        ${calendar_sets.calendar.subject} ${i + 1}일차
                    </a>
                    </div>`;
...
const setCalendarLayerData = async(e) => {
    try{
        await fetch(`${location.pathname}/calendar_by_id?_id=${e.dataset.calendar_id}`)
        .then(res => res.json())
        .then(data => {
            document.getElementById('calendar_id').value = data._id;
            ...
            document.getElementById('calendar_start_date').innerText = setDateFormat(data.start_date);
            document.getElementById('calendar_end_date').innerText = setDateFormat(data.end_date);
            console.log(document.getElementById('onlyowner').dataset.user_id);
            if(document.getElementById('onlyowner').dataset.user_id == data.user_id){
...
const setDetailData = async(YMD) => {
    ...
    deleteDetailData();
    await fetch(location.pathname + `/calendar?YMD=${YMD}`)
        .then((res) => res.json())
        .then((data) => {
            data.map(calendar_set => {
                calHtml += `<tr>
                <td>일정</td>
                <td>${calendar_set.calendar.subject} ${Math.abs(new Date(year, month - 1, day) - new Date(calendar_set.calendar.start_date)) / (1000 * 3600 * 24) + 1}일차</td>
                <td><div><a class='calendar' href='#calendar_layer' data-calendar_id=${calendar_set.calendar._id} onclick=setCalendarLayerData(this)>자세히보기</a></div></td>
                </tr>`
            });
        });
    if(calHtml == ''){
